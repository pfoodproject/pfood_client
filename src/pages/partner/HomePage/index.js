...
const Homepage = () => {
    ...
    useEffect(() => {
        setIsLoading(true);
        let partnerid = store.getState().partnerInfo.token.user.PartnerID;

        const fetchData = async (partnerid) => {
            const result = await callApiUnauthWithHeader(`partner/homepage`, 'GET', { partnerid: partnerid, year: year })
            setData(result.data);
        };
        fetchData(partnerid);
    }, [store, year]);
                    ...
                    <div>Loading...</div>
                        ...
                        <React.Fragment>
                            ...
                            <Grid
                                ...
                                xs={12}
                            >
                                <TotalUsers rsSourceOfItemsActive={data.rsSourceOfItemsActive} rsSourceOfItemsOfYear={data.rsSourceOfItemsOfYear} />
                            </Grid>
                            <Grid
                                item
                                lg={3}
                            ...
                            <Grid
                                ...
                                xs={12}
                            >
                                <LatestProducts rsLatestSourceOfItem={data.rsLatestSourceOfItem} />
                            </Grid>
                            <Grid
                                item
                                lg={8}
